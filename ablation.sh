...
declare -a representative_aggregations=("torch.mean")
declare -a deep_distance_types=("cosine" "euclidean" "l1")
    ...
    for distance_type in "${deep_distance_types[@]}"; do
        ...
        for layer_size in "${deep_distance_layer_sizes[@]}"; do
            ...
            for backbone_dim in "${backbone_layer_dims[@]}"; do
                ...
                python run.py exp.name=ablation method=relationnet dataset=tabula_muris n_way=5 n_shot=5 method.representative_aggregation=$aggregation method.deep_distance_type=$distance_type method.deep_distance_layer_sizes="$layer_size" backbone.layer_dim="$backbone_dim"
            done
        done
    done
done
